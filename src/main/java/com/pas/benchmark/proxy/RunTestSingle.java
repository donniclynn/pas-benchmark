...
public class RunTestSingle<T> extends DynamicProxySingle{
	 ...
	 * @param args
	 */
	public RunTestSingle(T realObject, Object[] paras) {
	 ...
	 * @param threads	线程数
	 */
	public  void runTest(RunTestSingle rt,int threads) {
		int j = 1;
		...
		Runtime.getRuntime().addShutdownHook(new Thread() {
			public void run() {
				...
				System.out.println("success Trans count: " + DynamicProxySingle.getTotalsuccessRecords().get());
				System.out.println("fail Trans count: " + DynamicProxySingle.getTotalfailRecords().get());
				System.out.println("Avg TPS: " + DynamicProxySingle.getTotalsuccessRecords().get() / (du / 1000));
				System.out.println("*******************************************************");
				result(null, "Avg TPS:" + DynamicProxySingle.getTotalsuccessRecords().get() / (du / 1000) + ":" + rtStr
	...
	public String countTPS(int interaval) {
		...
		timer.schedule(new TimerTask() {
			public void run() {
				...
				tmp = DynamicProxySingle.getTotalsuccessRecords().get() - preOKTrans;
				preOKTrans = DynamicProxySingle.getTotalsuccessRecords().get();
				preFailTrans = DynamicProxySingle.getTotalfailRecords().get();
				...
				while (System.currentTimeMillis() - startTime >= runTime) {
					if (saveRt)
						rtStr = art(DynamicProxySingle.getVts());
					System.exit(0);
				}
			}
		}, interval, interval);
